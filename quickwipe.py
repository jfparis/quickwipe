...
def gen_seed():
    p1 = subprocess.Popen(["dd", "if=/dev/urandom", "bs=128", "count=1"], stdout=subprocess.PIPE)
...
def wipe(args):
    for file in args.files:
        ...
        if file_size < 1:
            print "file %s is empty. Cannot wipe it" % (file)
            break

        for i in xrange(args.iterations):
            fd = open(file,"rb+")
            fd.seek(0, 0)
            p1 = subprocess.Popen(["cat", "/dev/zero"], stdout=subprocess.PIPE)
